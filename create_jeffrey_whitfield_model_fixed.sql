...
-- Get the researcher profile ID and create the model
DO $$
DECLARE
  researcher_id uuid;
  model_id uuid;
...
BEGIN
  ...
  IF researcher_id IS NULL THEN
    RAISE EXCEPTION 'Researcher profile not found. Make sure to replace YOUR_USER_ID_HERE with actual user_id';
  END IF;
  ...
  RAISE NOTICE 'Model created successfully!';
...
-- Instructions:
-- 1. Sign up/sign in as Jeffrey Whitfield
-- 2. Run this query to get your user_id:
--    SELECT id FROM auth.users WHERE email = 'your-email@example.com';
-- 3. Replace 'YOUR_USER_ID_HERE' at the top of this script with that ID
-- 4. Run this entire script
-- 5. The Black-Scholes model will be available at /models
