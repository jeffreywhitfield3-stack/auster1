...
export default function ProfilePageClient({ username }: ProfilePageClientProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {/* Profile Header */}
      <div className="bg-neutral-800/50 backdrop-blur-sm border border-neutral-700 rounded-lg p-6 mb-6">
        <div className="flex items-start gap-6">
          {/* Avatar */}
          <div className="flex-shrink-0">
            {profile.avatar_url ? (
              ...
              <div className="w-24 h-24 rounded-full bg-neutral-700 flex items-center justify-center border-2 border-neutral-600">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                ...
                <p className="text-base text-neutral-400">@{profile.username}</p>
              </div>
              ...
              <div className="flex items-center gap-3">
                {isOwnProfile ? (
                  <Link
                    ...
                    className="px-4 py-2 border border-neutral-600 text-neutral-300 hover:text-white font-medium rounded-lg hover:bg-neutral-700 transition-colors text-sm"
                  >
                    Edit Profile
                  </Link>
                ) : (
            ...
            {/* Bio */}
            {profile.bio && (
              <p className="text-sm text-neutral-300 mb-4 whitespace-pre-wrap leading-relaxed">
                {profile.bio}
              </p>
            )}
            ...
            {/* Stats */}
            <div className="flex items-center gap-6 pt-4 border-t border-neutral-700">
              <div>
                <span className="font-semibold text-white">{profile.follower_count || 0}</span>
                <span className="text-sm text-neutral-400 ml-1">Followers</span>
              </div>
              <div>
                <span className="font-semibold text-white">{profile.following_count || 0}</span>
                <span className="text-sm text-neutral-400 ml-1">Following</span>
              </div>
              <div>
                <span className="font-semibold text-white">{profile.model_count || 0}</span>
                <span className="text-sm text-neutral-400 ml-1">Models</span>
              </div>
              <div>
                <span className="font-semibold text-white">{profile.research_count || 0}</span>
                <span className="text-sm text-neutral-400 ml-1">Research</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          {models.length === 0 ? (
            <div className="text-center py-12 bg-neutral-800/50 border border-neutral-700 rounded-lg">
              <svg className="mx-auto h-10 w-10 text-neutral-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
              <p className="text-sm text-neutral-400">No models yet</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {models.map((model) => (
                <Link
                  ...
                  className="block bg-neutral-800/50 border border-neutral-700 rounded-lg p-4 hover:border-neutral-600 transition-colors"
                >
        ...
        <div>
          {research.length === 0 ? (
            <div className="text-center py-12 bg-neutral-800/50 border border-neutral-700 rounded-lg">
              ...
              <p className="text-sm text-neutral-400">No research posts yet</p>
            </div>
          ) : (
